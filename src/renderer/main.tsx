  ...
  return <div className="app">
    ...
    <div className="workspace">
      ...
      <main>
        ...
        {page === 'library' && <>
          <div className="page-heading"><div><span className="eyebrow">YOUR GAME. YOUR SPACE.</span><h1>Ready when you are.</h1><p>One launcher. A separate home for every way you play.</p></div><span className="build-label">VANILLA PREVIEW <i /></span></div>
          <section className={`hero ${selected.profile}`}>
            <div className="hero-copy"><span className="eyebrow">{selected.profile === 'mcsr' ? 'SPEEDRUNNING / ISOLATED INSTANCE' : 'COMET PVP / ISOLATED INSTANCE'}</span><h2>{selected.name}<span>Minecraft {selected.version}</span></h2><p>{selected.profile === 'mcsr' ? 'A clean starting point for your next run. RSG pack integration comes next.' : 'Back to the versions that made every hit count. Your worlds, your settings, your game.'}</p><div className="hero-actions"><button className="primary" disabled={locked || !!state.running} onClick={() => void run({ type: 'launch', id: selected.id })}><Icon name="play" />{state.running === selected.id ? 'Game running' : locked ? 'Working…' : 'Launch game'}</button><span>Java Edition <b>·</b> Vanilla</span></div></div>
          ...
          <aside className="details"><span className="eyebrow">INSTANCE DETAILS</span><h2>{selected.name}</h2><dl><div><dt>Version</dt><dd>{selected.version}</dd></div><div><dt>Mod loader</dt><dd>None / vanilla</dd></div><div><dt>Memory</dt><dd>{state.settings.memoryMb / 1024} GB</dd></div><div><dt>Java runtime</dt><dd>{state.settings.javaPath ? 'Java 8 · selected' : 'Not configured'}</dd></div></dl><div className="isolation"><span className="isolation-dot" /><div><strong>A space of its own</strong><p>Mods, worlds, and settings never share a game directory with another instance.</p></div></div><p className="phase-note">{selected.profile === 'mcsr' ? 'RSG pack not installed. This vanilla instance cannot play MCSR Ranked yet.' : 'Built-in PvP mods and the right-shift menu are not included in this milestone.'}</p><div className="detail-buttons"><button disabled={locked || !!state.running} onClick={() => void run({ type: 'install', id: selected.id })}>Install / verify files</button><button disabled={locked} onClick={() => void run({ type: 'folder', id: selected.id })}>Open game folder <span>↗</span></button></div></aside></div>
        </>}
        {page === 'settings' && <><div className="page-heading"><div><span className="eyebrow">MAKE YOURSELF AT HOME</span><h1>Launcher settings</h1><p>Global defaults for this first vanilla build.</p></div></div><section className="settings-panel"><div className="settings-section"><h2>Microsoft account</h2><p>{state.account ? `Signed in as ${state.account.name}. Tokens are encrypted using Windows secure storage.` : 'Sign in with an account that owns Minecraft: Java Edition.'}</p><label>Comet application (client) ID<input value={draft.clientId} placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" onChange={e => setDraft({ ...draft, clientId: e.target.value.trim() })} /></label><p className="hint">Requires your own Entra public-client registration and Minecraft Services approval. No client secret is needed.</p><button disabled={locked} onClick={() => void run({ type: state.account ? 'logout' : 'login' })}>{state.account ? 'Sign out' : 'Sign in with Microsoft'}</button></div><div className="settings-section"><h2>Java & memory</h2><label>64-bit Java 8 executable<div className="path-row"><input readOnly value={state.settings.javaPath} placeholder="Choose java.exe from a Java 8 installation" /><button disabled={locked} onClick={() => void run({ type: 'java' })}>Browse…</button></div></label><p className="hint">All three versions use Java 8. Comet checks the version and architecture before launch. Runtime downloads are not included yet.</p><label>Maximum memory <span className="memory-value">{draft.memoryMb} MB</span><input type="range" min="1024" max="16384" step="512" value={draft.memoryMb} onChange={e => setDraft({ ...draft, memoryMb: Number(e.target.value) })} /></label></div><div className="settings-section"><h2>Launch behavior</h2><label className="checkbox"><input type="checkbox" checked={draft.minimizeOnLaunch} onChange={e => setDraft({ ...draft, minimizeOnLaunch: e.target.checked })} />Minimize Comet when the game process starts</label><p className="hint">Comet returns when the game exits. Closing the launcher while a game or operation is active minimizes it instead.</p></div><button className="primary" disabled={locked} onClick={() => void run({ type: 'settings', settings: draft })}>Save settings</button></section></>}
