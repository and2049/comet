...
function App(): React.JSX.Element {
  ...
  async function run(command: Command): Promise<void> {
    setError('');
    if (!window.comet) {
      setError('This is a browser preview. Run bun start for the desktop launcher.');
      return;
    }
  ...
  function choose(instance: Instance): void {
    select(instance.id);
  }
  const filtered = state.instances.filter(
    i =>
      (filter === 'all' || i.profile === filter) &&
      ...
      <div className="workspace">
        <header className="topbar">
          <div className="wordmark">
            COMET <span>/</span>{' '}
            <small>{page === 'library' ? 'Launchpad' : page === 'console' ? 'Game console' : 'Settings'}</small>
          </div>
            ...
            <>
              ...
              <div className="library-layout">
                <section className="library">
                  ...
                  {filtered.length === 0 && <p className="empty">No instances match your search.</p>}
                </section>
                <aside className="details">
                  ...
                  <dl>
                    ...
                    <div>
                      <dt>Mod loader</dt>
                      <dd>{selected.profile === 'mcsr' ? 'Fabric · MCSR Ranked pack' : 'None / vanilla'}</dd>
                    </div>
                    <div>
                      <dt>Memory</dt>
                    ...
                    <div>
                      <dt>Java runtime</dt>
                      <dd>{state.settings.javaPath ? 'Java 8 · custom' : 'Java 8 · managed by Comet'}</dd>
                    </div>
                  </dl>
                  <p className="phase-note">
                    {selected.profile === 'mcsr'
                      ? 'Mods follow the upstream MCSR Ranked pack. Mods you add are kept.'
                      : 'Built-in PvP mods are not included yet.'}
                  </p>
            ...
            <>
              ...
              <section className="settings-panel">
                ...
                <div className="settings-section">
                  ...
                  <p className="hint">
                    Comet downloads the Java runtime Mojang declares for each version. Choose a 64-bit Java 8 executable
                    only to override it.
                  </p>
                  <label>
