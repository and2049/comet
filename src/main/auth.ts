...
export interface Session { account: Account; accessToken: string; refreshToken: string; clientId: string }
export function sessionFrom(value: unknown): Session {
  const s = record(value);
  const a = record(s.account);
  return { account: { id: text(a.id), name: text(a.name) }, accessToken: text(s.accessToken), refreshToken: text(s.refreshToken), clientId: text(s.clientId) };
}
async function exchange(msa: Record<string, unknown>, clientId: string, signal?: AbortSignal): Promise<Session> {
  const post = async (url: string, body: unknown) => record(await json(url, {
  ...
  const entitlements = record(await json('https://api.minecraftservices.com/entitlements/mcstore', { headers, signal }));
  if (!Array.isArray(entitlements.items) || entitlements.items.length === 0) throw new Error('This account has no Minecraft Java entitlement.');
  ...
  if (!/^[a-f0-9]{32}$/i.test(account.id) || !/^\w{1,16}$/.test(account.name)) throw new Error('Invalid Minecraft profile.');
...
export async function login(clientId: string, showCode: (code: string) => void, signal: AbortSignal): Promise<Session> {
  ...
  const device = record(await json(`${authority}/devicecode`, {
    method: 'POST', body: new URLSearchParams({ client_id: clientId, scope }), signal,
  }));
  ...
  while (Date.now() < deadline) {
    ...
    const response = await fetch(`${authority}/token`, {
      method: 'POST', body: new URLSearchParams({ client_id: clientId, device_code: text(device.device_code), grant_type: 'urn:ietf:params:oauth:grant-type:device_code' }),
...
export async function refresh(session: Session): Promise<Session> {
  const msa = record(await json(`${authority}/token`, {
    method: 'POST', body: new URLSearchParams({ client_id: session.clientId, refresh_token: session.refreshToken, grant_type: 'refresh_token', scope }),
  }));
