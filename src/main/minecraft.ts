...
interface RuntimeManifest { files: Record<string, { type: string; executable?: boolean; target?: string; downloads?: { raw: Download } }> }
export interface RuntimeFiles { directories: string[]; files: (Download & { path: string; executable: boolean })[]; links: { path: string; target: string }[]; java: string }
export function runtimeFiles(manifest: RuntimeManifest): RuntimeFiles {
  ...
  for (const [relative, entry] of Object.entries(manifest.files)) {
    ...
    else if (entry.type === 'file' && entry.downloads) result.files.push({ ...entry.downloads.raw, path: relative, executable: entry.executable === true });
...
export async function installRuntime(root: string, java: Metadata['javaVersion'], report: (message: string) => void): Promise<string> {
  ...
  const { directories, files, links, java: executable } = runtimeFiles(JSON.parse(await readFile(manifestFile, 'utf8')) as RuntimeManifest);
  ...
  for (const link of links) {
    ...
    if (!path.resolve(path.dirname(location), link.target).startsWith(home + path.sep)) throw new Error('Unsafe runtime link.');
...
export async function verifyJava(javaPath: string, major = 8): Promise<void> {
  const result = await exec(javaExecutable(javaPath), ['-XshowSettings:properties', '-version'], { windowsHide: true, timeout: 15000 });
...
export function launchArguments(installation: Installation, settings: Settings, session: Session): string[] {
  ...
  const values: Record<string, string> = {
    ...
    launcher_name: 'Comet', launcher_version: '0.1.0', classpath: classpath.join(path.delimiter),
  ...
  return [
    ...
    metadata.mainClass, ...expand(gameArgs, values, target),
  ];
}
