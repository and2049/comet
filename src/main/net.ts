...
export function trustedUrl(value: string, hosts: string[]): string {
  ...
  if (url.protocol !== 'https:' || url.username || url.password || url.port || !hosts.includes(url.hostname)) throw new Error(`Untrusted download URL: ${url.hostname}`);
  return url.href;
}
...
export async function parallel<T>(items: T[], limit: number, work: (item: T, index: number) => Promise<void>): Promise<void> {
  let next = 0;
  let failed = false;
