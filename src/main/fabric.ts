...
export function packFiles(index: PackIndex, version: string): PackFile[] {
  if (index.formatVersion !== 1 || index.game !== 'minecraft' || index.dependencies.minecraft !== version)
    throw new Error('The MCSR pack does not target this instance version.');
  ...
  return index.files
    ...
    .map(file => {
      if (!file.path.startsWith('mods/') || !/^[\w.+-]+\.jar$/.test(file.path.slice(5)))
...
export async function installFabric(
  ...
  const profile = (await json(
    `https://meta.fabricmc.net/v2/versions/loader/${version}/${loader}/profile/json`,
  )) as FabricProfile;
...
export async function installPack(
  ...
  let previous: string[] = [];
  try {
    previous = (JSON.parse(await readFile(record, 'utf8')) as { files: string[] }).files;
  } catch (error) {
  ...
  await writeFile(
    record,
    JSON.stringify(
      { versionId: index.versionId, loader: index.dependencies['fabric-loader'], files: files.map(file => file.path) },
      null,
      2,
    ),
  );
...
export function mergeFabric(installation: Installation, fabric: Fabric): Installation {
  ...
  return {
    ...
    metadata: {
      ...
      arguments: { jvm: [...fabric.jvm, ...(metadata.arguments?.jvm ?? [])], game: metadata.arguments?.game ?? [] },
    },
  };
}
export async function withFabric(
  root: string,
  instance: Instance,
