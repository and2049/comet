  ...
  files: [
    {
      ...
      hashes: { sha1 },
      env: { client: 'required', server: 'unsupported' } as { client: string },
      downloads: [
...
describe('Fabric and pack resolution', () => {
  ...
  test('rewrites GitHub raw links and rejects untrusted mod hosts', () => {
    ...
    expect(rawUrl('https://cdn.modrinth.com/data/x/versions/y/mod.jar')).toBe(
      'https://cdn.modrinth.com/data/x/versions/y/mod.jar',
    );
    for (const url of [
  ...
  test('accepts only client mods inside the mods folder', () => {
    ...
    for (const path of [
      ...
      expect(() => packFiles({ ...index, files: [{ ...index.files[1], path }] }, '1.16.1')).toThrow(
        'Unsupported pack file',
      );
    }
    expect(() => packFiles({ ...index, files: [{ ...index.files[1], downloads: [] }] }, '1.16.1')).toThrow(
      'one download',
    );
  });
  ...
  test('puts Fabric ahead of the vanilla classpath and arguments', () => {
    ...
    const merged = mergeFabric(installation, {
